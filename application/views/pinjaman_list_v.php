...
<div id="tb" style="height: 35px;">
	...
	<div class="pull-right" style="vertical-align: middle;">
		...
		<a href="<?=base_url()?>pinjaman/export_excel" class="easyui-linkbutton" iconCls="icon-excel" plain="false">Ekspor</a>
	</div>
</div>
				...
				<td>
					<table>
						...
						<tr style="height:35px">
							...
							<td>:</td>
							<td>
								<input type="hidden" name="provinsi" id="provinsi" readonly="readonly" />
								<input type="text" id="provinsi_txt" name="provinsi_txt" style=" background:#eee; border-width:1; width:195px; height:23px" readonly="true" />
							</td>	
						</tr>
						<tr style="height:35px">
...
$(document).ready(function() {
	$('#jenis_id').change(function(){
		...
		.done(function(result) {
			...
			$('#biaya_adm').val(data.biaya_adm);
			$('#bunga').val(data.bunga);
			$('#provinsi').val(data.provinsi);
			$('#biaya_adm_txt').val(data.biaya_adm);
			$('#bunga_txt').val(data.bunga);
			$('#provinsi_txt').val(data.provinsi);
